...
export default function ErrorPage() {
    ...
    return (
        <div className="flex flex-col items-center justify-center h-screen w-screen">
            <h1 className="text-2xl font-bold">We couldn't process your authentication request</h1>
            {isErrorLoading ? <LoadingSpinner className="text-center" /> : (
                <pre className="text-sm text-gray-500">{JSON.stringify(flowError, null, 2)}</pre>
            )}
        </div>
    )
}
