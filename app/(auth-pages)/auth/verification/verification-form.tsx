...
import { ErrorBrowserLocationChangeRequired, ErrorGeneric, isResponseError, LoginFlow, UiNode, UiNodeAnchorAttributes, UiNodeInputAttributes, VerificationFlow } from "@ory/client-fetch";
...
const formSchema = z
    .object({
        flowState: z.string().optional().default("choose_method"),
...
const VerificationForm = ({ className, flow, updateFlow, isFlowLoading = false }: Props) => {
    const [goto] = useQueryState('goto', parseAsString.withDefault(""));
    const [codeParam, setCodeParam] = useQueryState('code', parseAsString.withDefault(""));
    ...
    React.useEffect(() => {
        ...
        const csrfToken = (getOryUiNodeByGroupAndName(flow.ui.nodes, 'default', 'csrf_token')?.attributes as UiNodeInputAttributes)?.value ?? "";
        const email = (getOryUiNodeByGroupAndName(flow.ui.nodes, 'code', 'email')?.attributes as UiNodeInputAttributes)?.value
            ?? emailParam
            ?? (session?.identity?.verifiable_addresses && session.identity.verifiable_addresses.length > 0 && session.identity.verifiable_addresses.find(address => !address.verified)?.value)
            ?? "";
    ...
    const onFlowSubmit = (data: z.infer<typeof formSchema>) => startTransition(async () => {
        if (!flow) return;
        try {
    ...
    const onCodeResend = () => startCodeResendTransition(async () => {
        if (!flow) return;
        try {
            const res = await ory.updateVerificationFlow({
                flow: flow.id,
    ...
    React.useEffect(() => {
        const msg = flow?.ui?.messages?.find(message => message.type === "error")?.text;
        if (msg) {
            form.setError('root', {
                message: msg,
            })
    ...
    const getFlowStateForm = () => {
        if ((flow?.state === 'sent_email' || form.watch("email").length > 0) && flow?.state !== 'passed_challenge') {
            return (
                <Form {...form}>
                    <form
                    ...
                    >
                        ...
                        <div className="grid gap-2">
                            {flow?.state === 'sent_email' && (
                                ...
                                >
                                    {isLoading || isFlowLoading ? <LoadingSpinner /> : <CheckIcon className={`h-4 w-4 ${flow?.state === 'passed_challenge' ? 'text-green-500' : ''}`} />}
                            ...
                            >
                                {isCodeResending || isFlowLoading ? <LoadingSpinner /> : <MailIcon className="w-4 h-4" />}
            ...
            const href = (flow?.ui?.nodes.find(node => node.group === 'code' && node.type === 'a' && (node.attributes as UiNodeAnchorAttributes).id === 'continue')?.attributes as UiNodeAnchorAttributes)?.href;
            return (
                <Button
                    type="button"
                    className="w-full"
                    onClick={() => window.location.href = href ?? process.env.NEXT_PUBLIC_DOMAIN + decodeURIComponent(flow?.return_to ?? "")}
                >
                    Continue
                </Button>
            )
