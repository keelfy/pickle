...
import { GameNoteStatus } from '@/lib/model/content-note'
...
export default function ContentNoteGridFilters({
  value,
  onChange,
  statusOptions,
}: Props) {
    ...
    setFilters((prev) => {
      ...
      if (prevValue) {
        return prev.map((filter) =>
          filter.name === 'status' ? { ...filter, value } : filter,
        )
      }
  ...
  const handleRequesterChange = (value: string) => {
    setFilters((prev) => {
      ...
      const prevValue = prev.find(
        (filter) => filter.name === 'requester',
      )?.value
      if (prevValue) {
        return prev.map((filter) =>
          filter.name === 'requester' ? { ...filter, value } : filter,
        )
      }
  ...
  const areFiltersChanged = React.useMemo(
    ...
    <div className="grid gap-4">
      <div className="grid gap-2">
        ...
        <Select
          ...
          value={
            filters.find((filter) => filter.name === 'status')?.value ?? 'any'
          }
          defaultValue="any"
        >
          <SelectTrigger>
          ...
          <SelectContent>
            {[{ value: 'any', label: 'Any status' }, ...statusOptions].map(
              (option) => (
                <SelectItem
                  key={option.value}
                  value={option.value}
                ...
                >
                  <div className="flex items-center gap-1">
                    <ContentNoteStatusIcon
                      status={option.value as GameNoteStatus}
                      className="size-4"
                    />
                    {option.label}
                  </div>
      ...
      <div className="grid gap-2">
        ...
        <Input
          ...
          value={
            filters.find((filter) => filter.name === 'requester')?.value ?? ''
          }
