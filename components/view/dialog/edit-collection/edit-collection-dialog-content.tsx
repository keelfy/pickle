...
export default function EditCollectionDialogContent() {
  ...
  const collectionState = React.useMemo(() => {
    return collectionStates.find(
      (state) => state.collection.id === modalParams?.id,
    )
  ...
  return (
    <>
      <DialogHeader>
        <DialogTitle>{collectionState?.collection.name}</DialogTitle>
        <DialogDescription>
          You can use collections to organize your content the way you want.
        </DialogDescription>
      </DialogHeader>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="mt-4 flex w-full flex-col gap-2">
            ...
            {collectionState.page < collectionState.totalPages - 1 && (
              <EditCollectionDialogLoadMoreItems
                collectionId={collectionState.collection.id}
              />
            )}
            <FormDescription>
              Note that item deletion does not require confirmation and will be
              instantly applied.
            </FormDescription>
          </div>
