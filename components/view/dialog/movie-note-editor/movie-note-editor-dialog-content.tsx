...
import { GameNote, GameNoteStatus, MovieNoteStatus } from "@/utils/api/types";
...
import { MovieNote } from "@/utils/api/types";
...
export default function MovieNoteEditorDialogContent({ noteId }: Props) {
    ...
    React.useEffect(() => {
        ...
        fetchContentNote<MovieNote>(profile, "movies", noteId, 'lg')
            .then(setMovieNote)
            .catch((err) => {
                console.error(err);
                toast({
    ...
    const onSubmit = form.handleSubmit((values) => {
        ...
        startTransition(async () => {
            try {
                const res = await updateContentNote<MovieNote, Partial<CreateGameNoteReq>>(
                    profile,
                    "movies",
                    noteId,
                    values
                        ...
                        <div className="flex items-start space-x-4">
                            ...
                            <div className="grid min-h-[225px] w-full">
                                ...
                                <div className="flex items-center gap-3 mt-auto mb-1">
                                    ...
                                    {movieNote?.content.websites && getSourceLinks(movieNote.content.websites)}
                                </div>
                            </div>
                        </div>
