...
import { ResponseError, SettingsFlow, UiNodeInputAttributes, UpdateSettingsFlowBody } from "@ory/client-fetch";
...
export default function EmailChangeForm({ className, flow, updateFlow }: Props) {
    ...
    const updateFormFromSettingsFlow = (flow: SettingsFlow) => {
        const emailNode = flow?.ui?.nodes?.find(node => node.group === 'profile' && (node.attributes as UiNodeInputAttributes).name === 'traits.email');
        const csrfTokenNode = flow?.ui?.nodes?.find(node => node.group === 'default' && (node.attributes as UiNodeInputAttributes).name === 'csrf_token');

        form.reset({
            ...form.getValues(),
            csrf_token: (csrfTokenNode?.attributes as UiNodeInputAttributes)?.value as string,
            traits: {
                email: (emailNode?.attributes as UiNodeInputAttributes)?.value as string,
            },
        });
            ...
            flow.continue_with.forEach(continueWith => {
                if (continueWith.action === 'show_verification_ui') {
                    if (continueWith.flow.url) {
                        ...
                        router.push(`/auth/verification?flow=${continueWith.flow.id}&email=${continueWith.flow.verifiable_address}`);
                    }
                }
            });
        }
    ...
    const updateErrors = (flow: SettingsFlow) => {
        const flowNodes = flow?.ui?.nodes?.filter(node => node.group === 'profile');
        const emailNode = flowNodes?.find(node => (node.attributes as UiNodeInputAttributes).name === 'traits.email');

        if (emailNode?.messages?.length && emailNode.messages.filter(message => message.type === 'error').length > 0) {
            form.setError("traits.email", {
                message: emailNode.messages.filter(message => message.type === 'error')[0].text,
            })
        } else {
            form.clearErrors("traits.email");
        }

        const messagesNode = flow.ui.messages?.find(message => message.type === 'error');
        if (messagesNode) {
            form.setError("root", {
    ...
    const handleSubmit = form.handleSubmit(async (values) => startTransition(async () => {
        try {
            ...
            >
                ...
                {isSuccess && (
                    <p className="text-sm text-green-500">
                        Email change request sent. Please check your email for a verification code.
                    </p>
                )}
            </form>
        </Form >
