...
export default function ProviderIdentityElement({
    ...
    const isLinked = (flow?.ui?.nodes
        ?.find(node => node.group === 'oidc' && (node.attributes as UiNodeInputAttributes).value?.toLowerCase() === providerType)
    ...
    const onClick = () =>
        startTransition(async () => {
            ...
            const attrs = flow.ui?.nodes?.find(node => node.group === 'oidc' && (node.attributes as UiNodeInputAttributes).value?.toLowerCase() === providerType)?.attributes as UiNodeInputAttributes;
            try {
                ...
                if (e?.response?.status === 422) {
                    try {
                        ...
                        const errorMessage = errorBody?.ui?.messages?.[0]?.text ||
                            errorBody?.ui?.nodes?.find((node: any) => node.messages?.length > 0)?.messages[0]?.text ||
