...
export default function PasswordChangeForm({
  ...
  const updateFormFromSettingsFlow = (flow: SettingsFlow) => {
    ...
    form.reset({
      csrfToken: (csrfTokenNode?.attributes as UiNodeInputAttributes)
        ?.value as string,
      newPassword: (passwordNode?.attributes as UiNodeInputAttributes)
        ?.value as string,
      confirmNewPassword: (passwordNode?.attributes as UiNodeInputAttributes)
        ?.value as string,
    })
  ...
  const updateErrors = (flow: SettingsFlow) => {
    ...
    const passwordNode = flowNodes?.find(
      (node) => (node.attributes as UiNodeInputAttributes).name === 'password',
    )

    if (
      passwordNode?.messages?.length &&
      passwordNode.messages.filter((message) => message.type === 'error')
        .length > 0
    ) {
      form.setError('newPassword', {
        message: passwordNode.messages.filter(
  ...
  return (
    <Form {...form}>
      <form className={cn('grid space-y-2', className)} onSubmit={handleSubmit}>
        ...
        <div className="grid w-full gap-1 pt-2">
