...
import { ErrorBrowserLocationChangeRequired, isResponseError, SettingsFlow, UiNodeInputAttributes } from "@ory/client-fetch";
...
export default function TwitchIntegrationSettingsTab() {
    ...
    const isTwitchLinked = (flow?.ui?.nodes
        ?.find(node => node.group === 'oidc' && (node.attributes as UiNodeInputAttributes).value?.toLowerCase() === 'twitch-extended')
        ?.attributes as UiNodeInputAttributes)?.name === 'unlink';
    ...
    const handleTwitchConnect = () => startConnectingTransition(async () => {
        if (!flow) return;

        const attrs = flow.ui?.nodes?.find(node => node.group === 'oidc' && (node.attributes as UiNodeInputAttributes).value?.toLowerCase() === 'twitch-extended')?.attributes as UiNodeInputAttributes;
        try {
            ...
            if (isResponseError(e)) {
                if (e.response.status === 400) {
                    ...
                    const res = await e.response.json() as ErrorBrowserLocationChangeRequired;
                ...
                toast({
                    title: "Error",
                    description: e?.message || "Error linking provider",
                    variant: "destructive",
                });
            }
        }
        ...
        <div className="flex flex-col justify-between h-full space-y-6 w-full">
            <div className="grid gap-2">
                <div className="flex items-center gap-2">
                    ...
                    />
                    <h1 className="text-lg font-bold">Twitch Channel Points Integration</h1>
                </div>
                <div className="flex space-x-4 justify-between items-center border rounded-md p-4">
                    <div className="flex-1 flex flex-col space-y-2 text-sm">
                        You can provide access to channel points redemptions from your Twitch channel to automatically populate your orders.
                    </div>
                    <Button
                        variant="outline"
                    ...
                    >
                        {isConnecting || isFlowPending ? <LoadingSpinner /> : (isTwitchLinked ? <XIcon className="w-4 h-4 text-destructive" /> : <KeyIcon className="w-4 h-4" />)}
                        {isFlowPending ? "Waiting..." : (isTwitchLinked ? "Disconnect" : "Connect")}
                    </Button>
                </div>
            </div>
