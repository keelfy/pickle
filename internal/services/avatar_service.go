...
type AvatarService interface {
	GetAvatarUrlById(ctx context.Context, profile *db.Profile, size string) (*string, error)
	UploadAvatarForPreviewById(ctx context.Context, profile *db.Profile, file multipart.File, fileHeader *multipart.FileHeader, size string) (string, error)
	ConfirmProfileAvatar(ctx context.Context, profile *db.Profile) (string, error)
}
...
func (service *avatarService) GetAvatarUrlById(ctx context.Context, profile *db.Profile, size string) (*string, error) {
	if _, ok := avatarSizes[size]; !ok {
		return nil, errors.NewBadRequestError("Invalid avatar size", nil)
	}

	cacheKey := fmt.Sprintf("avatar:%s:%s", profile.UserID, size)
...
func (service *avatarService) UploadAvatarForPreviewById(ctx context.Context, profile *db.Profile, file multipart.File, fileHeader *multipart.FileHeader, size string) (string, error) {
	...
	fileName := fmt.Sprintf("%s%s", profile.UserID, ext)
	...
	// update profile
	updatedProfile, err := service.sqlDb.Queries().UpdateProfilePreviewAvatarByUserId(ctx, db.UpdateProfilePreviewAvatarByUserIdParams{
...
func (service *avatarService) ConfirmProfileAvatar(ctx context.Context, profile *db.Profile) (string, error) {
	bucketName := config.GetAvatarBucketName()
