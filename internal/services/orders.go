...
func NewOrderService(pgxpool *pgxpool.Pool, queries *db.Queries, userService *User) *Order {
	return &Order{
		pgxpool:     pgxpool,
		queries:     queries,
...
func (service *Order) GetSortedByReceiverId(ctx context.Context, receiverId uuid.UUID, sort *utils.CursorSort) ([]*db.Order, error) {
...
const findOrdersByReceiverIdQuery = `SELECT * FROM "orders" WHERE "receiver_id" = $1 AND "%s" %s $2 ORDER BY "%s" %s LIMIT $3`

func (service *Order) GetOrderById(ctx context.Context, id uuid.UUID) (*db.Order, error) {
	orders, err := service.queries.FindOrderById(ctx, id)
...
func (service *Order) findSortedOrdersByReceiverId(ctx context.Context, receiverID uuid.UUID, sort *utils.CursorSort) ([]*db.Order, error) {
	comparisonOperator := "<"
	...
	rows, err := service.pgxpool.Query(ctx, query, receiverID, sort.Cursor, sort.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (service *Order) ApproveOrderByIdAndReceiverId(ctx context.Context, orderId, receiverId, initiatorUserId uuid.UUID) (*db.Order, error) {
	...
	initialOrder, err := service.queries.FindOrderById(ctx, orderId)
	if err != nil {
	...
	// Approve the order
	initialOrder, err = service.queries.UpdateOrderById(ctx, db.UpdateOrderByIdParams{
		ID:        initialOrder.ID,
		UpdatedAt: time.Now(),
...
func (service *Order) ApproveOrder(ctx context.Context, order *db.Order, initiatorUserID uuid.UUID) (*db.Order, error) {
	// Approve the order
	approvedOrder, err := service.queries.UpdateOrderById(ctx, db.UpdateOrderByIdParams{
		ID:        order.ID,
		Status:    types.OrderStatus_Approved,
		UpdatedBy: initiatorUserID,
		UpdatedAt: time.Now(),
...
func (service *Order) CreateOrder(ctx context.Context, userId uuid.UUID, req *types.CreateOrderReq) (*db.Order, error) {
	...
	createdOrder, err := service.queries.InsertOrder(ctx, db.InsertOrderParams{
		CreatedBy:       userId,
		UpdatedBy:       userId,
		ReceiverID:      receiver.UserID,
		PaymentType:     req.PaymentType,
		Amount:          req.Amount,
		OrdererUsername: &req.OrdererUsername,
...
// Creates an instantly approved ordered
func (service *Order) CreateApprovedOrder(ctx context.Context, receiverId, orderedBy uuid.UUID, category int, message string) (*db.Order, error) {
	createdOrder, err := service.queries.InsertOrder(ctx, db.InsertOrderParams{
		CreatedBy:   orderedBy,
		ReceiverID:  receiverId,
		PaymentType: types.PaymentType_None,
		Amount:      0,
		Status:      types.OrderStatus_Approved,
		OrderedBy:   &orderedBy,
	...
	if err != nil {
		return nil, errors.NewInternalServerError("Error occurred during initial order creation", err)
	}
	return createdOrder, nil
}
...
func (service *Order) UpdateOrderById(ctx context.Context, orderId uuid.UUID, req *types.UpdateOrderReq, userId uuid.UUID) (*db.Order, error) {
	...
	updatedOrder, err := service.queries.UpdateOrderById(ctx, db.UpdateOrderByIdParams{
		ID:        order.ID,
		UpdatedAt: time.Now(),
		UpdatedBy: userId,
		Status:    req.Status,
